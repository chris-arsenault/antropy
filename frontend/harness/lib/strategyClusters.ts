...
import { traitValues } from "./genotypeTraits";
...
export function strategyClusters(world: EngineWorld, k = 3): StrategySnapshot {
  if (!Number.isSafeInteger(k) || k < 2 || k > 8) throw new Error("Cluster count must be 2–8");
  ...
  for (const [rank, j] of ranked.entries()) {
    ...
    clusters.push({
      rank,
      size: members.length,
      leftBand: members.filter((i) => cells[i].x < config.width / 2).length,
