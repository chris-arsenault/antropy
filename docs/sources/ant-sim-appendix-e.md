...
| 7   | **Write / derive the weights**             | —                      | Hand-write isolated reflex-grade entries. Derive their integration from the zero-state oracle as balanced, shuffled, independent sensor/output frames: reset hidden state for every frame, initialize and hold recurrence at zero during initial training, and train no outcome objective. Recurrent loci remain in the genome for later evolution; the initializer does not need them to imitate a same-frame target | Weights exist; each locus is tagged hand-written or trained; every predetermined initialization receives the same fixed frame-distillation procedure on randomized teacher worlds. No controller receives terminal polishing or individual acceptance                                                                                    |
...
| 10  | **Seeded colony closes the loop**          | —                      | Full worker population produced by the one step-7 initial-training procedure. Apply it once to predetermined independent initializations, then evaluate every resulting controller without selection. No oracle runs in evaluated worlds, no closed-loop optimizer follows cloning, and no controller receives individual intervention or acceptance                                                                  | At least three quarters of the independently trained controllers occupy step 5's viable region and complete the food loop on held-out worlds. The gap to the programmed reference is retained evolutionary headroom, not an optimization target                                                                                          |
