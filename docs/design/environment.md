...
colony founding, weather, seasons, decay, spoil hauling, automatic continuation, and the authored
entrance trail remain independently isolatable. Cargo-capacity overrides use `null` for
genome-derived production values and explicit per-world values only for logged experiments.
...
[Appendix G §G.2](../sources/ant-sim-appendix-g.md), and
[Appendix H §H.2](../sources/ant-sim-appendix-h.md).
