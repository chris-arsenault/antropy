...
it("resolves body contact across the periodic seam", () => {
  ...
  resolveContacts(w);
  expect(distance(w.cells[0], w.cells[1], w.config)).toBeCloseTo(0.9, 10);
  expect(w.cells.every((c) => c.contacts.some((v) => v === 1))).toBe(true);
});
it("bounded locomotion cannot tunnel through a stationary cell", () => {
  const w = createWorld(1, { ...DEFAULT_CONFIG, founders: 2, rotationalDiffusion: 0 });
  ...
  expect(distance(w.cells[0], w.cells[1], w.config)).toBeGreaterThanOrEqual(0.9 - 1e-10);
});
