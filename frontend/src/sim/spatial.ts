...
export class SpatialIndex {
  ...
  constructor(
    ...
    const size = Math.max(2, radius(2 * config.birthMass, config) * 2);
  ...
  free(p: Point, mass: number, ignore: number): boolean {
    return this.near(p).every(
      (other) =>
        other.id === ignore ||
        distance(p, other, this.config) >=
          radius(mass, this.config) + radius(other.mass, this.config)
    );
  }
}
